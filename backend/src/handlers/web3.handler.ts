...
import { WEB3_OPTS } from '../config/web3.config'
import MailHandler from './mail.handler'
const supportChildrenContract = require('../../client/src/contracts/SupportChildren.json')
const deployedNetwork = supportChildrenContract.networks[WEB3_OPTS.networkId]

const web3 = new Web3(
    new Web3.providers.WebsocketProvider(WEB3_OPTS.websocketProvider)
)
...
contract.events.DonationMade({}, async (error:any, data:any) => {
    if (error) {
    ...
    if (calcPerc.notifyDonorsOnReachedPercentage) {
        ...
        mail.campaignReachedInterestedPercentage(campaignId, calcPerc.collectedAmount, emails, (await filledPercentage(campaignId, 0)).perc)
    }
})

contract.events.CampaignFinished({}, async (error:any, data:any) => {
    if (error) {
        return console.log('Error: ' + error)
    }
    ...
    mail.campaignFinished(campaignId, collectedAmount, emails, (await filledPercentage(campaignId, 0)).perc)
})

async function filledPercentage(campaignId:any, donationAmount:any) {
